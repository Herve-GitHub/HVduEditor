...
function CanvasList.new(parent, props)
    ...
    self._content_height = 400  -- 内容区域高度（可根据需要调整）
    -- 创建主容器（浮动窗口样式）
...
function CanvasList:_create_title_bar()
    ...
    self.title_label:set_text("图页选择")
...
-- 添加自定义工具
function CanvasList:add_tool(tool_def)
    table.insert(self._tools, tool_def)
end

-- 获取所有工具
function CanvasList:get_tools()
    return self._tools
end

-- 检查是否正在拖拽工具
function CanvasList:is_dragging_tool()
    return self._tool_drag_state.is_dragging
end

-- 获取当前拖拽的工具
function CanvasList:get_dragging_tool()
